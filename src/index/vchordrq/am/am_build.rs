...
fn make_internal_build_with_tracking(
    ...
    for w in internal_build.lists.iter().rev().copied().chain(once(1)) {
        ...
        // Resolve and verify id_text for representatives now to avoid CTID drift later
        let mut resolved_level_reps = level_representatives;
        for rep in &mut resolved_level_reps {
            let id_val = rep
                .id_text
                .clone()
                .or_else(|| id_from_map(id_map, rep.doc_id))
                .or_else(|| fetch_heap_id_by_ctid(heap_relid_for_ids, rep.doc_id));
            if let Some(id_txt) = id_val {
                if !verify_heap_id_exists(heap_relid_for_ids, &id_txt) {
                    pgrx::error!(
                        "failed to verify existence of id {} for centroid representative",
                        id_txt
                    );
                }
                rep.id_text = Some(id_txt);
            } else {
                let fallback = format_doc_hash(rep.doc_id);
                pgrx::error!(
                    "failed to verify existence of id {} for centroid representative",
                    fallback
                );
    ...
    for (level_idx, structure) in structures.iter().enumerate() {
        ...
        for (cluster_id, count, percentage, centroid_rep) in entries {
            if let Some(rep) = centroid_rep {
                ...
                let display_id = images_id.unwrap_or_else(|| format_doc_hash(rep.doc_id));
                pgrx::info!(
...
fn verify_heap_id_exists(heap_relid: pgrx::pg_sys::Oid, id_text: &str) -> bool {
    unsafe {
        let rel = pgrx::pg_sys::RelationIdGetRelation(heap_relid);
        if rel.is_null() {
            return false;
        ...
        let _ = pgrx::spi::Spi::connect(|client| {
            ...
            let has_id = client
                .select(check_sql, None, &[heap_relid.into()])
                .map(|r| r.len() > 0)
                .unwrap_or(false);
            if has_id {
                let sql = format!("SELECT 1 FROM {} WHERE id = $1::text LIMIT 1", qualified);
                if let Ok(rows) = client.select(&sql, None, &[id_text.into()]) {
                    exists = rows.len() > 0;
                }
            } else {
                ...
                if let Ok(pk_rows) = client.select(pk_sql, None, &[heap_relid.into()]) {
                    if pk_rows.len() > 0 {
                            ...
                            .unwrap_or(None)
                        {
                            ...
                            let sql = format!(
                                "SELECT 1 FROM {} WHERE {} = $1::text LIMIT 1",
...
fn fetch_heap_id_by_ctid(heap_relid: pgrx::pg_sys::Oid, doc_id: (u16, u16, u16)) -> Option<String> {
    ...
    unsafe {
        ...
        if value.is_none() {
            pgrx::info!(
                "id-lookup: unable to resolve id for CTID ({},{},{})",
                doc_id.0,
                doc_id.1,
                doc_id.2
            );
