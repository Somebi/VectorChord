    ...
    for (level_idx, structure) in structures.iter().enumerate() {
        ...
        for (cluster_id, count, percentage, centroid_rep) in entries {
            if let Some(rep) = centroid_rep {
                // Attempt to resolve id for centroid representative
                let images_id = rep
                    .id_text
                    .clone()
                ...
                // Verify that we obtained a real id and that it exists in the heap table
                match images_id {
                    Some(ref id) if verify_id_exists(heap_relid, id) => {
                        pgrx::info!(
                    ...
                    Some(id) => {
                        pgrx::error!(
                            "failed to verify existence of id {id} for centroid representative"
                        );
                    }
                    None => {
                        pgrx::error!("failed to resolve id for centroid representative");
                    }
                }
            } else {
                pgrx::info!(
...
fn verify_id_exists(heap_relid: pgrx::pg_sys::Oid, id_text: &str) -> bool {
    unsafe {
        let rel = pgrx::pg_sys::RelationIdGetRelation(heap_relid);
        if rel.is_null() {
            return false;
        ...
        let _ = pgrx::spi::Spi::connect(|client| {
            ...
            if let Ok(rows) = client.select(&sql_only, None, &[id_text.into()]) {
                if rows.len() > 0 {
                    exists = true;
                    return Ok::<_, pgrx::spi::Error>(());
                }
            ...
            if let Ok(rows) = client.select(&sql_all, None, &[id_text.into()]) {
                if rows.len() > 0 {
                    exists = true;
                }
            }
