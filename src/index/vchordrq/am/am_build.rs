        ...
        for (cluster_idx, centroid) in centroids.iter().enumerate() {
            ...
            for (input_idx, input_vector) in input.iter().enumerate() {
                ...
                if distance < best_distance {
                    ...
                    // Resolve id from map or heap and verify it exists in the table
                    let id_text = resolve_id_with_verification(doc_id, id_map, heap_relid_for_ids);
    ...
    for (level_idx, structure) in structures.iter().enumerate() {
        ...
        for (cluster_id, count, percentage, centroid_rep) in entries {
            if let Some(rep) = centroid_rep {
                // Resolve and verify id for centroid representative
                let images_id = rep
                    .id_text
                    .clone()
                    .or_else(|| resolve_id_with_verification(rep.doc_id, id_map, heap_relid));
                pgrx::info!(
...
fn verify_id_exists(heap_relid: pgrx::pg_sys::Oid, id: &str) -> bool {
    unsafe {
        let rel = pgrx::pg_sys::RelationIdGetRelation(heap_relid);
        if rel.is_null() {
            return false;
        ...
        let _ = pgrx::spi::Spi::connect(|client| {
            let sql = format!(
                "SELECT 1 FROM ONLY {} WHERE id = $1::text LIMIT 1",
                qualified
            );
            exists = client
                ...
                .unwrap_or(false);
            if !exists {
                let sql_all = format!("SELECT 1 FROM {} WHERE id = $1::text LIMIT 1", qualified);
                exists = client
                    .select(&sql_all, None, &[id.into()])
                    .map(|r| r.len() > 0)
                    .unwrap_or(false);
            }
            Ok::<_, pgrx::spi::Error>(())
...
fn resolve_id_with_verification(
    doc_id: (u16, u16, u16),
    id_map: &HashMap<(u32, u16), String>,
    heap_relid: pgrx::pg_sys::Oid,
) -> Option<String> {
    let id_opt = id_from_map(id_map, doc_id).or_else(|| fetch_heap_id_by_ctid(heap_relid, doc_id));
    if let Some(id) = id_opt {
        if verify_id_exists(heap_relid, &id) {
            Some(id)
        } else {
            pgrx::error!("failed to verify existence of id {id} for centroid representative");
            None
        }
    } else {
        pgrx::error!(
