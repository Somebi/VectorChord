...
pub unsafe extern "C-unwind" fn ambuild(
    ...
    let structures = match vchordrq_options.build.source.clone() {
        ...
        VchordrqBuildSourceOptions::Internal(internal_build) => {
            ...
            let (samples, mut sample_origins) = 'a: {
                ...
                heap.traverse(false, |(ctid, store)| {
                    for (vector, extra) in store {
                        ...
                        if number_of_samples < max_number_of_samples {
                            ...
                            // Do NOT fetch id here; resolve after sampling using the id_map
            ...
            // Resolve real ids for all sampled origins now so metadata uses stable ids
            for origin in &mut sample_origins {
                let doc_id = (origin.0, origin.1, origin.2);
                if origin.4.is_none() {
                    origin.4 = id_from_map(&id_map, doc_id)
                        .or_else(|| fetch_heap_id_by_ctid(heap_relid, doc_id));
                }
            }
